...
function usage() {
	...
	console.error(
		"  dist-dir: path to GoReleaser dist/ containing release archives",
	);
	process.exit(1);
}
...
function smokeTest(binPath, version) {
	...
	if (r.error)
		throw new Error(`smoke test failed for ${binPath}: ${r.error.message}`);
	if (r.status !== 0)
		throw new Error(`smoke test exited ${r.status} for ${binPath}`);
	if (!r.stdout.includes(version)) {
		throw new Error(
			`smoke test version mismatch for ${binPath}: expected ${version}, got ${r.stdout}`,
		);
	}
}
...
function main() {
	...
	for (const t of targets) {
		...
		if (t.os === process.platform && t.cpu === process.arch) {
			...
			console.log(
				`  skipping smoke test for ${name} (not runnable on ${process.platform}/${process.arch})`,
			);
		}
	...
	const templateKeys = Object.keys(
		mainTemplate.optionalDependencies || {},
	).sort();
