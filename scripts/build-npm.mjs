...
function main() {
	...
	// Ship the pi extension shim alongside the CLI shim. pi auto-discovers
	// extensions/ from the "pi" manifest in package.json; the .ts file is
	// compiled at load by pi's runtime, so no build step is needed here.
	const extSrc = "npm/claude-statusline/extensions/pi-statusline.ts";
	if (existsSync(extSrc)) {
		const extDir = join(mainDir, "extensions");
		mkdirSync(extDir, { recursive: true });
		cpSync(extSrc, join(extDir, "pi-statusline.ts"));
	} else {
		throw new Error(`pi extension source not found: ${extSrc}`);
	}
