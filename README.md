...
- **Multiple Claude Code profiles.** If you run more than one Claude Code login on the same machine (e.g. a shell function that sets `CLAUDE_CONFIG_DIR` for a second account), set `claude_config_dir` to the profile you want the shim to track:
  ```toml
  [quota_shim]
  enabled = true
  claude_config_dir = "~/.claude-personal"
  ```
  How this works: Claude Code scopes its macOS keychain item per config dir. The default `~/.claude` profile uses the service name `Claude Code-credentials`; a profile launched with `CLAUDE_CONFIG_DIR` set gets `Claude Code-credentials-` plus the first 8 hex chars of `sha256` of the config-dir string (e.g. `sha256("/Users/you/.claude-personal")[:8]`). Each profile's login lives under its own service name — which means the *unscoped* entry can belong to a **different account** entirely (say, a work login in `~/.claude` and a personal one in `~/.claude-personal`). Without `claude_config_dir`, a statusline rendering inside the scoped profile would read the unscoped keychain entry and happily report the *other* account's quota — same-looking bars, wrong numbers. Setting it makes the shim read the scoped service (and `<dir>/.credentials.json` on Linux/Windows) for exactly the profile you name; the env var `CLAUDE_CONFIG_DIR` is used as a fallback when the config key is unset. Resolution order: `claude_config_dir` → `$CLAUDE_CONFIG_DIR` → default profile.

  The shim's cache and lock files are keyed the same way — the default profile keeps `quota-shim.json` / `quota-shim.lock`, a scoped profile gets `quota-shim-<hash8>.json` / `quota-shim-<hash8>.lock` — so sessions on different profiles never overwrite each other's cached windows. Leave `claude_config_dir` unset for a single-profile setup — behavior (and filenames) are unchanged. `agents-statusline quota` prints the resolved profile and keychain service name so you can confirm it's reading the right one.
