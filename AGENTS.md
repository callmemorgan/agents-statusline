...
- **OAuth quota shim** (`internal/quota`) — opt-in `[quota_shim]` bridge that fills the Fable/Sonnet/Opus weekly windows until Claude Code's statusline payload carries them. Detached `quota-refresh` worker (spawn pattern mirrors update-check: lock + `sys.ApplyDetachSysProcAttr`) fetches `api.anthropic.com/api/oauth/usage` with the Claude Code OAuth token (env → macOS keychain → `.credentials.json`; the token is never written to disk) and caches normalized model-scoped windows at `$XDG_STATE_HOME/agents-statusline/quota-shim.json` (profile-scoped: `[quota_shim].claude_config_dir` or `$CLAUDE_CONFIG_DIR` selects a Claude Code profile, which suffixes the keychain service, cache, and lock names with `-` + `sha256(configDir)[:8]` — the default profile keeps the legacy unsuffixed names; resolution is in-process, no subprocesses on the render path). On render, `quota.MaybeInject` (one `os.ReadFile`, before `state.Record` so projections work) fills `RateLimits.ModelScoped` — a shim-only field (`json:"-"`); the statusline wire has no model-class windows and the binary deliberately does not parse any (add real parsing in `internal/payload` if Claude Code ever ships them). `agents-statusline quota` is the foreground verification subcommand. The endpoint is unofficial: all failures are soft (empty cache → segments hide).
- **Foreign subscription usage** (`internal/foreignusage`) — reads the sanitized Codex/Grok/Antigravity/Kimi cache. After rendering, `MaybeRefresh` checks the cache mtime against `[foreign_usage].refresh_minutes` (default 1) and, when stale or absent, spawns one detached `foreign-usage-refresh` worker guarded by `foreign-usage.lock`. The worker runs `claude-all-usage` from beside the installed binary (or `PATH`); the current render keeps stale data and the next invocation sees the atomic rewrite. Failures touch an existing cache to prevent retry storms.
